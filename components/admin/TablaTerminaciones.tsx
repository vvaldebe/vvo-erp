'use client'

import { useState, useTransition } from 'react'
import { Plus, Check, X, Pencil, Scissors } from 'lucide-react'
import { crearTerminacion, actualizarTerminacion, toggleActivaTerminacion } from '@/app/actions/terminaciones'
import type { Terminacion, UnidadMedida } from '@/types/database.types'

function formatCLP(amount: number): string {
  return new Intl.NumberFormat('es-CL', {
    style: 'currency',
    currency: 'CLP',
    minimumFractionDigits: 0,
  }).format(amount)
}

interface EditState {
  id: string | null   // null = nueva fila
  nombre: string
  unidad: UnidadMedida
  precio: string
}

const EMPTY_EDIT: EditState = { id: null, nombre: '', unidad: 'm2', precio: '' }

interface Props {
  terminaciones: Terminacion[]
}

export default function TablaTerminaciones({ terminaciones }: Props) {
  const [editando, setEditando] = useState<EditState | null>(null)
  const [error, setError] = useState<string | null>(null)
  const [isPending, startTransition] = useTransition()

  function startNew() {
    setEditando({ ...EMPTY_EDIT })
    setError(null)
  }

  function startEdit(t: Terminacion) {
    setEditando({ id: t.id, nombre: t.nombre, unidad: t.unidad, precio: String(t.precio) })
    setError(null)
  }

  function cancel() {
    setEditando(null)
    setError(null)
  }

  function save() {
    if (!editando) return
    const precio = parseFloat(editando.precio)
    if (!editando.nombre.trim()) { setError('El nombre es requerido'); return }
    if (isNaN(precio) || precio < 0) { setError('Precio inválido'); return }

    startTransition(async () => {
      const data = { nombre: editando.nombre.trim(), unidad: editando.unidad, precio }
      const result = editando.id
        ? await actualizarTerminacion(editando.id, data)
        : await crearTerminacion(data)

      if ('error' in result) {
        setError(result.error)
        return
      }
      setEditando(null)
      setError(null)
    })
  }

  function handleToggle(id: string, activo: boolean) {
    startTransition(async () => {
      await toggleActivaTerminacion(id, activo)
    })
  }

  const inputCls = 'px-2 h-8 text-[13px] bg-[var(--bg-card)] border border-[var(--border-default)] rounded-[5px] text-[var(--text-primary)] focus:outline-none focus:border-[#7c3aed] focus:ring-2 focus:ring-[#7c3aed]/10 transition-colors w-full'

  return (
    <div className="bg-[var(--bg-card)] border border-[var(--border-default)] rounded-[8px] overflow-hidden">

      {/* Header */}
      <div className="px-4 h-11 border-b border-[var(--border-default)] flex items-center justify-between">
        <p className="text-[13px] text-[var(--text-secondary)]">
          {terminaciones.length} terminación{terminaciones.length !== 1 ? 'es' : ''}
        </p>
        {!editando && (
          <button
            type="button"
            onClick={startNew}
            className="inline-flex items-center gap-1.5 px-3 h-7 bg-[#7c3aed] hover:bg-[#6d28d9] text-white text-[12px] font-medium rounded-[5px] transition-colors cursor-pointer"
          >
            <Plus className="w-3.5 h-3.5" />
            Nueva terminación
          </button>
        )}
      </div>

      {/* Tabla */}
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead>
            <tr className="bg-[var(--bg-muted)] border-b border-[var(--border-default)]">
              <th className="text-left px-4 h-9 text-[11px] font-medium text-[var(--text-secondary)] uppercase tracking-wide">Nombre</th>
              <th className="text-left px-4 h-9 text-[11px] font-medium text-[var(--text-secondary)] uppercase tracking-wide">Unidad</th>
              <th className="text-right px-4 h-9 text-[11px] font-medium text-[var(--text-secondary)] uppercase tracking-wide">Precio</th>
              <th className="text-center px-4 h-9 text-[11px] font-medium text-[var(--text-secondary)] uppercase tracking-wide">Activo</th>
              <th className="text-right px-4 h-9 text-[11px] font-medium text-[var(--text-secondary)] uppercase tracking-wide"></th>
            </tr>
          </thead>
          <tbody>
            {/* Fila de nueva terminación */}
            {editando?.id === null && (
              <tr className="border-b border-[#7c3aed]/20 bg-[#ede9fe]/20">
                <td className="px-4 py-2">
                  <input
                    type="text"
                    placeholder="Nombre de la terminación"
                    value={editando.nombre}
                    onChange={(e) => setEditando({ ...editando, nombre: e.target.value })}
                    className={inputCls}
                    autoFocus
                  />
                </td>
                <td className="px-4 py-2">
                  <select
                    value={editando.unidad}
                    onChange={(e) => setEditando({ ...editando, unidad: e.target.value as UnidadMedida })}
                    className={inputCls}
                  >
                    <option value="m2">m²</option>
                    <option value="ml">ml</option>
                    <option value="unidad">Unidad</option>
                  </select>
                </td>
                <td className="px-4 py-2">
                  <input
                    type="number"
                    min={0}
                    step={1}
                    placeholder="0"
                    value={editando.precio}
                    onChange={(e) => setEditando({ ...editando, precio: e.target.value })}
                    className={`${inputCls} text-right`}
                  />
                </td>
                <td className="px-4 py-2 text-center text-[var(--text-muted)] text-[11px]">—</td>
                <td className="px-4 py-2 text-right">
                  <div className="flex items-center justify-end gap-2">
                    {error && (
                      <span className="text-[11px] text-[#dc2626] mr-1">{error}</span>
                    )}
                    <button
                      type="button"
                      onClick={save}
                      disabled={isPending}
                      className="inline-flex items-center gap-1 px-3 h-7 text-[12px] font-medium text-white bg-[#7c3aed] rounded-[5px] hover:bg-[#6d28d9] transition-colors disabled:opacity-60 cursor-pointer"
                    >
                      <Check className="w-3 h-3" />
                      Guardar
                    </button>
                    <button
                      type="button"
                      onClick={cancel}
                      className="inline-flex items-center gap-1 px-3 h-7 text-[12px] font-medium text-[var(--text-secondary)] border border-[var(--border-default)] rounded-[5px] hover:bg-[var(--bg-muted)] transition-colors cursor-pointer"
                    >
                      <X className="w-3 h-3" />
                      Cancelar
                    </button>
                  </div>
                </td>
              </tr>
            )}

            {terminaciones.length === 0 && editando?.id !== null ? (
              <tr>
                <td colSpan={5} className="px-4 py-14 text-center">
                  <div className="flex flex-col items-center gap-2 text-[var(--text-muted)]">
                    <Scissors className="w-8 h-8 opacity-25" />
                    <p className="text-[13px]">No hay terminaciones aún</p>
                    <p className="text-[12px] text-[var(--text-faint)]">Crea la primera con el botón de arriba</p>
                  </div>
                </td>
              </tr>
            ) : (
              terminaciones.map((t) => {
                const isEditing = editando?.id === t.id
                return (
                  <tr
                    key={t.id}
                    className={[
                      'border-b border-[var(--border-subtle)] last:border-0 transition-colors',
                      isEditing ? 'bg-[#ede9fe]/20 border-[#7c3aed]/20' : 'hover:bg-[var(--bg-muted)]',
                      !isEditing ? 'bg-[var(--bg-card)]' : '',
                    ].join(' ')}
                  >
                    <td className="px-4 h-11">
                      {isEditing ? (
                        <input
                          type="text"
                          value={editando.nombre}
                          onChange={(e) => setEditando({ ...editando, nombre: e.target.value })}
                          className={inputCls}
                          autoFocus
                        />
                      ) : (
                        <span className="text-[13px] font-medium text-[var(--text-primary)]">{t.nombre}</span>
                      )}
                    </td>
                    <td className="px-4 h-11">
                      {isEditing ? (
                        <select
                          value={editando.unidad}
                          onChange={(e) => setEditando({ ...editando, unidad: e.target.value as UnidadMedida })}
                          className={inputCls}
                        >
                          <option value="m2">m²</option>
                          <option value="ml">ml</option>
                          <option value="unidad">Unidad</option>
                        </select>
                      ) : (
                        <span className="inline-flex items-center px-2 py-0.5 rounded-[4px] text-[11px] font-mono font-medium bg-[#f4f4f5] text-[var(--text-secondary)]">
                          {t.unidad}
                        </span>
                      )}
                    </td>
                    <td className="px-4 h-11 text-right">
                      {isEditing ? (
                        <input
                          type="number"
                          min={0}
                          step={1}
                          value={editando.precio}
                          onChange={(e) => setEditando({ ...editando, precio: e.target.value })}
                          className={`${inputCls} text-right`}
                        />
                      ) : (
                        <span className="text-[14px] tabular-nums text-[var(--text-primary)]">
                          {formatCLP(t.precio)}
                        </span>
                      )}
                    </td>
                    <td className="px-4 h-11 text-center">
                      {isEditing ? (
                        <span className="text-[var(--text-muted)] text-[11px]">—</span>
                      ) : (
                        <label className="relative inline-flex items-center cursor-pointer">
                          <input
                            type="checkbox"
                            checked={t.activo}
                            onChange={(e) => handleToggle(t.id, e.target.checked)}
                            disabled={isPending}
                            className="sr-only peer"
                          />
                          <div className="w-8 h-4 rounded-full transition-colors peer-checked:bg-[#7c3aed] bg-[#d4d4d8] peer-disabled:opacity-60" />
                          <div className="absolute left-0.5 top-0.5 w-3 h-3 bg-[var(--bg-card)] rounded-full shadow transition-transform peer-checked:translate-x-4" />
                        </label>
                      )}
                    </td>
                    <td className="px-4 h-11 text-right">
                      {isEditing ? (
                        <div className="flex items-center justify-end gap-2">
                          {error && (
                            <span className="text-[11px] text-[#dc2626] mr-1">{error}</span>
                          )}
                          <button
                            type="button"
                            onClick={save}
                            disabled={isPending}
                            className="inline-flex items-center gap-1 px-3 h-7 text-[12px] font-medium text-white bg-[#7c3aed] rounded-[5px] hover:bg-[#6d28d9] transition-colors disabled:opacity-60 cursor-pointer"
                          >
                            <Check className="w-3 h-3" />
                            Guardar
                          </button>
                          <button
                            type="button"
                            onClick={cancel}
                            className="inline-flex items-center gap-1 px-3 h-7 text-[12px] font-medium text-[var(--text-secondary)] border border-[var(--border-default)] rounded-[5px] hover:bg-[var(--bg-muted)] transition-colors cursor-pointer"
                          >
                            <X className="w-3 h-3" />
                            Cancelar
                          </button>
                        </div>
                      ) : (
                        <button
                          type="button"
                          onClick={() => startEdit(t)}
                          disabled={!!editando}
                          className="inline-flex items-center gap-1.5 px-3 h-7 text-[12px] font-medium text-[var(--text-secondary)] border border-[var(--border-default)] rounded-[5px] hover:bg-[var(--bg-muted)] hover:text-[var(--text-primary)] transition-colors disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
                        >
                          <Pencil className="w-3 h-3" />
                          Editar
                        </button>
                      )}
                    </td>
                  </tr>
                )
              })
            )}
          </tbody>
        </table>
      </div>
    </div>
  )
}
